...
class SimObject: 
	def __init__(self, T1, T2, K , TD):
		...
		self.b_2=(K/(T1-T2))*(self.alpha_1*T2*(1-self.alpha_2)-self.alpha_2*T1*(1-self.alpha_1))
	...
	def init_M(self, S_list, n, n_u):
		...
		for i in range(n):
			for j in range(i+1 if i+1<=n_u else n_u):
				arr[i][j]= S_list[i+1-j] if i+1-j+1<=len(S_list) else S_list[-1]
...
if __name__ == "__main__":
	...
	y_zad_list=[y_zad for i in range(iterations)]

	ax.plot(time, sim_object1.y_list, color='r')
	ax.plot(time, sim_object2.y_list, color='b')
